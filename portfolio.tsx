...
const projects = [
  ...
  { number: "05", title: "Service operations platform for Liebherr Saudi", tag: "Workflow automation · Service operations", text: "Rebuilt a single-file calculator into a multi-user service-order platform that carries a job from an engineer’s phone on site to the invoice. I modelled engineer, supervisor, and timekeeper handoffs, customer signatures, hour classification, expense tracking, party-specific invoicing, audit history, and controlled correction requests so the record stays trusted from start to finish.", outcome: "Saved double work by making one signed record drive the entire process" },
]
